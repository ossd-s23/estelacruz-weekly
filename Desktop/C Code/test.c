...
void queueAddTask (char *content, int starting, int size){
...
void rle(task_t *currentTask){
    ...
    while(count < chunkSize){   //count range: 0, filebytesize - 1, so we have according nec indices
        ...
        lastCount = currentcharCount;
         ...
         currentTask->encodedResults[writeIndex] = mychar;
         //encodedData[writeIndex] = mychar;
         //resultsIndex++;
         writeIndex++;
         currentTask->encodedResults[writeIndex] = unsignedcount;
    ...
    currentTask->outputSize = writeIndex;
    currentTask->lastChars[0] = currentTask->encodedResults[writeIndex-2];
    currentTask->lastChars[1] = currentTask->encodedResults[writeIndex-1];
...
/*
        if(displayed == 0){
            //first time
            //printf("%s\n","---FINALLY PRINTING---"); 
            //checkChunk = taskResults->numberofChunks;
            checkChunk = taskResults->outputSize;
            if((taskResults->outputSize) >= 3){
                //there us more than one chunk so we have to stitch!
                fwrite(encodedData, sizeof(unsigned char),(taskResults->outputSize)-2, stdout);
                //fwrite(taskResults->encodedResults, sizeof(unsigned char),(taskResults->outputSize)-2, stdout);
                lastTwoChars[0] = taskResults ->lastChars[0];
                lastTwoChars[1] = taskResults ->lastChars[1];
                previousCount = taskResults ->lastCharCount;
