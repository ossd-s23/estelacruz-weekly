...
// used to get the BASENAME of the current working dir
//Check: if in the root dir, then token if "/"
...
void shellworkingdir() {
   ...
   //printf("%s %s %s", "Base Name: '", baseName, "'");
   //printf("\n");
   // replace space at the end, will still have space, this is undefined behavior
   //baseName[strlen(baseName)] = ']';
    // replace space at the end, will still have space
   //baseName[strlen(baseName)] = '$';
...
int main(int argc, char  **argv) {
   ...
   // is it okay for it to be outside while
   ...
   while(1) {
      ...
      */
      ...
      //** NOT FINISHED BUILT IN COMMANDS: FG AND JOBS**  
      ...
      //prints a list of currently suspended jobs to STDOUT, one job per line.
      ...
      //example: [1] ./hello NEWLINE [2] /usr/bin/top -c NEWLINE [3] cat > output.txt

      //There are no more than 100 suspended jobs at one time.
      //There are no pipes in any suspended jobs.

      //The only way to resume a suspended job is by using the fg command (see below)
      //The jobs command takes no arguments.
      //If it is called with any arguments,
      ...
      if (strcmp(commandName, "jobs") == 0) {
         ...
         //printf("%s\n", "jobs command");
         continue;
         // exit once we incorp while loop
         //check num of args
      }
      
      
  
      //Built In fg Function: Able to resume a child
      //This command resumes a job in the foreground.
      // It takes exactly one argument: the job index, 
      //which is the number inside the bracket printed by the jobs command
      //fg 2 resumes /usr/bin/top -c

      // have to update job indexes: Push everything up
      //Note that the job index of cat > output.txt would become 2 as a result. 
      // Should the job /usr/bin/top -c be suspended again, it would be inserted to the end of the job list

      //If fg is called with 0 or 2+ arguments, 
      //your shell should print the following error message to STDERR and prompt for the next command.
      //Error: invalid command

      //If the job index does not exist in the list of currently suspended jobs, 
      //your shell should print the following error message to STDERR and prompt for the next command.
      //Error: invalid job

     
      if (strcmp(commandName, "fg") == 0) {
         //built in cmds cant be redirected or piped
         if(check_io_pipe > 0){
            printf("%s\n", "Error: Invalid Command");
            continue;
      ...
      //if there are currently suspended jobs, your shell should not terminate.
   ...
   */
   ...
   }else if (outputRedir_overwrite > 0 && outputRedir_append == 0 && inputRedirCount == 0 && pipeCount == 0 )
   {
      ...
      */
      ...
      // int clO = strlen(cpyFirst);
      //printf("%s%s%s\n", "string value = '", cpyFirst, "'");
      //printf("%s%d\n", "string len is: ", clO);
      //printf("%s%c%s\n", "last char is '", cpyFirst[strlen(cpyFirst) - 1], "'");
      //printf("%s%c%s\n", "very last char is '", cpyFirst[strlen(cpyFirst)], "'");
      //cpyFirst[strlen(cpyFirst)] = '\0';
      //printf("%s%d\n", "h is ", h);
      /*
      first[h] = NULL;
      ...
      for(h = 2; h < cmdCount; h++){
         second[h] = commandArr[h];
         //printf("%s%s%s\n", "array value = '", second[h], "'");
         strcat(cpySec, second[h]);
         if(h != cmdCount-1){
            strcat(cpySec, " "); //add space
         }
         printf("%s%s%s\n", "string value = '", cpySec, "'");
      }
      int cS = strlen(cpySec); 
      //printf("%s%s%s\n", "string value = '", cpySec, "'");
      //printf("%s%d\n", "string len is: ", cS);
      //printf("%s%c%s\n", "last char is '", cpySec[strlen(cpySec) - 1], "'");
      // printf("%s%c%s\n", "very last char is '", cpySec[strlen(cpySec)], "'");
      second[h] = NULL;
      if(strcmp(second[3], "<") == 0){
         //printf("%s\n", "input second");
         int checkIn = inputRedirection(cpySec, inputRedirCount, second);
      }else if (strcmp(second[3], ">>") == 0)
      {
         //printf("%s\n", "output second");
         int check_oo = outputRedirection_append(cpySec, outputRedir_append, second);
      }
      */
